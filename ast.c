...
Node makeAST(Node parseTreeNode, Node parentAST)
{
    ...
    else if (!parseTreeNode->isterminal) //if node of parsetree is non terminal
    {
        ...
        else if (datatype == functionDef || datatype == stmt || datatype == arithmeticExpression || datatype == arithmeticExpression2 || datatype == typevar || datatype == matrixVar || datatype == matrixRows || datatype == matrixRow || datatype == booleanExpression || datatype == elseStmt || datatype == constrainedVars)
        {
            ...
            if (datatype == arithmeticExpression || datatype == arithmeticExpression2 || datatype == constrainedVars)
            {
