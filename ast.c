...
Node makeAST(Node parseTreeNode, Node parentAST)
{
    ...
    else if (!parseTreeNode->isterminal) //if node of parsetree is non terminal
    {
        ...
        // if (datatype == arithmeticExpression2)
        // {
        //     printf("Helo\n");
        // }
        ...
        else if (datatype == functionDef || datatype == stmt || datatype == arithmeticExpression || datatype == arithmeticExpression2 || datatype == typevar || datatype == matrixVar || datatype == matrixRows || datatype == matrixRow)
        {
            ...
            // else if (datatype == matrixRows)
            // {
            //     rebalanceMatrixRow(curnode->child); //do something
            // }
        }

        if (datatype == declarationStmt || datatype == sizeStmt || datatype == conditionalStmt || datatype == elseStmt || datatype == ioStmt || datatype == funCallStmt || datatype == assignmentStmt)
        {
    ...
    else
    {
        ...
        else if (datatype == MAIN || datatype == INT || datatype == REAL || datatype == STRING || datatype == MATRIX || datatype == PLUS || datatype == MINUS || datatype == MUL || datatype == DIV)
        {
